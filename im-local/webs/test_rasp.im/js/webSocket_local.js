...
var localSocket = lSocket.Method = { 
    ...
    disconnect:true,
    msg:null,  
    mkLoginStr(user, passwd){
        this.user = user
        datas = {"login":user, "password":passwd} 
        this.msg  = JSON.stringify(datas)  
    },
    mkRouteMsg(data){
        datas = {"from":this.user, "to":"admin@rasp", "data":data} 
        this.msg =  JSON.stringify(datas)  
    },
    send:function(){
        if (this.msg == null) {
    ...
    connect:function(url){   //"wsLocal://localhost:8080/msg"
        ...
        wsLocal.onmessage = function (event) { 
            ...
            try{        
                data = tbase64.decode(event.data) 
                data = JSON.parse(data)   
                bodysRight.addMsg(data)
            }catch(err){
                console.debug(event.data)
